...
public class GroupController {
	...
	ResponseEntity<CommonResponseDto<?>> updateGroupMember(@Parameter(description = "그룹 id", required = true)
		@PathVariable @Validated Integer groupId, @Parameter(description = "변경할 멤버", required = true)
		@RequestBody @Validated GroupMemberReq groupMemberReq){
	...
	ResponseEntity<CommonResponseDto<?>> updateGroupSequence(@Parameter(description = "그룹 id 리스트", required = true)
		@RequestBody @Validated GroupListReq groupListReq){
	...
	@PathVariable @Validated Integer groupId){
		List<Integer> groupMemberList = groupService.selectGroupMember(groupId);
		
		//1. mes db에서 사용자 조회 API 요청
