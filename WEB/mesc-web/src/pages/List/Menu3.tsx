...
export const Menu3 = () => {
  ...
  const [showActionIdQuery, setShowActionIdQuery] =
    useRecoilState(ShowActionIdQuery);
  // body
  const [body, setBody] = useRecoilState(DataListBody);
  // 저장하고 useEffect 돌리기 위한것
  const [addDataListSavePoint, setAddDataListSavePoint] =
    useRecoilState(AddDataListSavePoint);
  ...
  const saveButton = () => {
    api
      .post("component/admin/4", { componentList: [body] })
      .then((res) => {
            ...
            <S.RightBody>
              <S.BlockHeader>
                <S.InputBox color={C.colors.buttonBlueBackground}>
                  <S.Input
                    type="text"
                    placeholder="데이터 제목을 작성해주세요."
                    onChange={(e) => handleBlockTitleChange(e.target.value)}
                  />
                </S.InputBox>
